...
if not 3 <= nargs <= 5:
    ...
    username = "mastermnkca"
    password = "mastermnkcaprogrammers2016*"
    host = "52.25.254.77"
    ...
    remote_path = '/var/www/mnkca/master/html/src/AppBundle-dev-' + sys.argv[1] + '.tar.gz'



    # CREATE TAR ON STAGE
    print ("Creating the tar archive for downloading from stage..." + "\n")
    commands = ["cd /var/www/mnkca/master/html/; grunt build; cd src; sudo tar czvf AppBundle-dev-" + sys.argv[1] + ".tar.gz AppBundle"]
