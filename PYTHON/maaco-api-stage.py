...
if not 2 <= nargs <= 5:
    ...
    remote_path = '/var/www/maacoapi/html/src/Acme-dev-' + sys.argv[1] + '.tar.gz'



    # CREATE TAR ON STAGE
    print ("Creating the tar archive for downloading from stage..." + "\n")
    commands = ["cd /var/www/maacoapi/html/; cd src; sudo tar czvf Acme-dev-" + sys.argv[1] + ".tar.gz Acme"]
